...
function generateOfferFile(answers: Record<string, string>): string {
  const lines: string[] = [];
  lines.push("---");
  ...
  if (answers.offer_name) {
    lines.push("## The Offer");
    lines.push("");
    lines.push(answers.offer_name.trim());
    lines.push("");
  }

  if (answers.offer_outcome) {
    lines.push("## The Outcome");
    lines.push("");
    lines.push(answers.offer_outcome.trim());
    lines.push("");
  }

  if (answers.offer_price) {
    lines.push("## Price & Model");
    lines.push("");
    lines.push(answers.offer_price.trim());
    lines.push("");
  }

  if (answers.offer_audience) {
    lines.push("## Who It's For");
    lines.push("");
    lines.push(answers.offer_audience.trim());
    lines.push("");
  }

  if (answers.offer_differentiator) {
    lines.push("## Why This vs Alternatives");
    lines.push("");
    lines.push(answers.offer_differentiator.trim());
    lines.push("");
  }
...
export default function OfferPreviewPage() {
  ...
  const offerFile = generateOfferFile(answers);
  const { enrichedContent, isLoading, error: enrichError, enrich, reset: resetEnrichment, isEnriched } = useEnrichment("offer", answers);
  const displayContent = enrichedContent || offerFile;
  const wordCount = offerFile.split(/\s+/).filter(Boolean).length;
      ...
      <div className="mx-auto max-w-6xl px-6 py-8">
        ...
        {view === "split" ? (
          <div className="grid gap-6 lg:grid-cols-2">
            ...
            <div style={{ border: "1px solid #1a1a1a", backgroundColor: "#111111" }}>
              <TerminalHeader title="offer.md" color="#22c55e" />
              <div className="p-6">
                {isLoading && (
