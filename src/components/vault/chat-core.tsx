  ...
  async function sendMessage(text?: string) {
    ...
    try {
      ...
      setMessages([
        ...
        {
          role: "assistant",
          content: "Connection error. Is the Pocket Architect running?",
        },
      ]);
    }
              ...
              <div className="flex flex-wrap justify-center gap-2 mt-4">
                {quickActions.map((action) => (
                  <button
                    ...
                    className="px-3 py-1.5 text-xs bg-surface border border-border rounded-lg text-muted hover:text-foreground hover:border-blue/40 transition-colors"
                  >
                    {action.label}
                  </button>
                ))}
